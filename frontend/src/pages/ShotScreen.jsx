...
const ShotScreen = () => {
  ...
  const { isSessionOpen, startSession, endSession, sessionData, userID } =
    useSession();
  ...
  const URL_StomachSocket = "ws://172.20.10.5:81";
  ...
  const calculatePrecision = (impacts, maxDistance = MAX_DISTANCE) => {
    ...
    const total = impacts.reduce(
      (sum, shot) => sum + computeShotPrecision(shot, maxDistance),
      0
    );
  ...
  useEffect(() => {
    ...
    if (headImpacts.length > 0)
      targetPrecisions.push(calculatePrecision(headImpacts));
    if (stomachImpacts.length > 0)
      targetPrecisions.push(calculatePrecision(stomachImpacts));
    if (RSImpacts.length > 0)
      targetPrecisions.push(calculatePrecision(RSImpacts));
    if (LSImpacts.length > 0)
      targetPrecisions.push(calculatePrecision(LSImpacts));

    if (targetPrecisions.length === 0) {
      setSessionPrecision("0%");
    } else {
      const avg = Math.round(
        targetPrecisions.reduce((sum, p) => sum + p, 0) /
          targetPrecisions.length
      );
  ...
  const sendM4Impact = (targetName) => {
    ...
    if (
      m4SocketRef.current &&
      m4SocketRef.current.readyState === WebSocket.OPEN
    ) {
  ...
  const sendLEDMessage = (target, command) => {
    ...
    switch (target) {
      case "Head":
        if (
          headSocketRef.current &&
          headSocketRef.current.readyState === WebSocket.OPEN
        ) {
      ...
      case "Stomach":
        if (
          stomachSocketRef.current &&
          stomachSocketRef.current.readyState === WebSocket.OPEN
        ) {
  ...
  const shotWrongTarget = (data, receivedTarget) => {
    if (shotTimerRef.current) clearTimeout(shotTimerRef.current);
    console.log(
      `Tir sur mauvaise cible : ${receivedTarget} reçu alors que ${currentTarget} était attendu – Score: 0`
    );
    const prec = computeShotPrecision({ x: data.x, y: data.y });
    setShotPrecision(prec + "%");
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      console.error(
        "Erreur lors de l'enregistrement de la séquence de tir",
        err
      );
    }
  };
  ...
  return (
    <div className="relative">
      <div
        className={`space-y-6 ${
          !isSessionOpen ? "blur-sm pointer-events-none" : ""
        }`}
      >
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="w-full flex gap-6">
            <div className="flex-1">
              <InfoBox
                icon="📊"
                titleKey="sessionPrecision"
                value={sessionPrecision}
              />
            </div>
            <div className="flex-1">
              <InfoBox
                icon="📊"
                titleKey="shotPrecision"
                value={shotPrecision}
              />
            </div>
            <div className="flex-1">
